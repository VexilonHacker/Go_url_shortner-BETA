...
func check_file() {
	...
	if os.IsNotExist(err) {
		// File doesn't exist, create and write initial data
		createAndInitFile()
		...
		// File exists and size > 0, check if CSV can be read properly
		...
		if err != nil || len(content) == 0 {
			// CSV read failed or file is empty in terms of CSV content, re-init file
			createAndInitFile()
		}
	}
}

func createAndInitFile() {
